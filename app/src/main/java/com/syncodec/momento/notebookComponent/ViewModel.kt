...
class ViewModel(application: Application) : AndroidViewModel(application) {
	...
	var notebook = mutableStateOf(
		Notebook(
			primaryKey = ""
		)
	)
	var chapterList = mutableStateListOf<Chapter>()
	var noteList = mutableStateListOf<Note>()
	var currentRoute = mutableStateListOf<String>()
	var currentRouteName = mutableStateListOf<String>()
	...
	fun observeNotebook() {
		fileObserver = object : FileObserver(
			...
			override fun onEvent(event: Int, file: String?) {
				Log.i("npr71", "fileObserver : $event")
			...
			try {
				withContext(Dispatchers.IO) {
					...
					notebook.value.apply {
						...
						this.noteMap = tmpNotebook.noteMap
						this.isFavourite = tmpNotebook.isFavourite
						this.isArchived = tmpNotebook.isArchived
						this.isLocked = tmpNotebook.isLocked

						this@ViewModel.chapterList.removeAll { true }
						tmpNotebook.chapterMap.values.forEach {
							this@ViewModel.chapterList.add(it)
						}

						this@ViewModel.noteList.removeAll { true }
						tmpNotebook.noteMap.values.forEach {
							this@ViewModel.noteList.add(it)
						}
					}
