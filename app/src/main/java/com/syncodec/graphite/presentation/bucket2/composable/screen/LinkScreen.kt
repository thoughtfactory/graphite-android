...
private fun DataGridView(
    ...
    BucketItemGridContainer(
        ...
        val bucketItemLink by remember(key1 = bucketItemBox?.bucketItemData) { derivedStateOf { bucketItemBox?.bucketItemData as? BucketItemLink } }

        LinkGridCard(
            bucketItemBox = bucketItemBox,
            bucketItemLink = bucketItemLink,
            isReorderable = pageNumber == 0,
            onClick = { bucketItemBox ?: return@LinkGridCard; if (isSelecting) selectionContainerActor.selectItem(objectBoxId = bucketItemBox.id) else onClickBucketItem(bucketItemBox) },
            onLongClick = { bucketItemBox ?: return@LinkGridCard; selectionContainerActor.selectItem(objectBoxId = bucketItemBox.id) },
            dragHandle = {
