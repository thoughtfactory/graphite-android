...
import com.syncodec.graphite.utils.alice.getData
import com.syncodec.graphite.utils.alice.putData
		...
		CompositionLocalProvider(
			...
			LocalAuthenticatorAction provides {
				if (isVaultOpened) {
					...
					val alice = context.getData("passcode")
					...
					authenticator = when (it) {
						...
						Authenticator.CHANGE_PASSCODE -> if (alice.result == AliceRequestResult.SUCCESS) Authenticator.AUTHENTICATE else Authenticator.ADD_PASSCODE
						Authenticator.REMOVE_PASSCODE -> if (alice.result == AliceRequestResult.SUCCESS) Authenticator.AUTHENTICATE else Authenticator.ADD_PASSCODE
				...
				AnimatedContent(targetState = authenticator) { target ->
					when (target) {
						Authenticator.AUTHENTICATE -> AuthenticatorScreen(
							onAuthentication = {
								val passcode = context.getData("passcode").data?.decodeToString()
						...
						Authenticator.ADD_PASSCODE -> AddPasscodeScreen(
							onPasscodeAdded = {
								context.putData("passcode", it.toByteArray())
								authenticator = Authenticator.NONE
