...
fun AddPasscodeScreen(
	...
	var passcodeState by remember { mutableStateOf(AddPasscodeState.NewPasscode) }
	...
	Column(
		...
		AnimatedContent(
			...
			label = "passcodeState_animation"
		) {
			Text(
				text = when (it) {
					AddPasscodeState.NewPasscode -> "Enter new passcode"
					AddPasscodeState.ConfirmPasscode -> "Confirm passcode"
					AddPasscodeState.WrongPasscode -> "Passcode doesn't match. Enter new passcode"
				},
		...
		PasscodeNumPad {
			when (passcodeState) {
				AddPasscodeState.NewPasscode -> {
					passcode1 = it
					passcodeState = AddPasscodeState.ConfirmPasscode
				}

				AddPasscodeState.ConfirmPasscode -> {
					passcode2 = it
					if (passcode1 == passcode2) onAddPasscode(passcode1) else passcodeState = AddPasscodeState.WrongPasscode
				}

				AddPasscodeState.WrongPasscode -> {
					passcode1 = it
					passcodeState = AddPasscodeState.ConfirmPasscode
				}
			}
