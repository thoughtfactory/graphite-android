...
import com.syncodec.graphite.presentation.exp.meta.AbstractExploreViewModel
...
class CalendarViewModel(repositoryStateFlow: MutableStateFlow<Repository.Companion.RepositoryStatus>) : AbstractExploreViewModel(repositoryStateFlow) {
	...
	init {
		viewModelScope.launch(Dispatchers.Default) {
			combine(super.filteredNoteList, this@CalendarViewModel._latLngBound) { filteredNoteList1, latLngBound ->
				Pair(filteredNoteList1, latLngBound)
			}.collectLatest { (filteredNoteList1, latLngBound) ->
				if (latLngBound != null) {
					this@CalendarViewModel._contextFilteredNoteList.tryEmit(filteredNoteList1.filter { noteObjectLite -> noteObjectLite.latLng?.toGLatLng()?.let { latLngBound.contains(it) } ?: false })
				} else {
					this@CalendarViewModel._contextFilteredNoteList.tryEmit(listOf())
				}
			}
		}
	}
