package com.syncodec.graphite.presentation.common.scrollBar
...
//				val firstItemSize = firstVisibleItem.size
				...
				val knobSize = fixedKnobRatio?.let { it * viewportSize } ?: ((viewportSize * viewportSize) / estimatedFullListSize)
				...
				drawRoundRect(
					...
					size = if (horizontal) Size(size.width - padding.toPx() * 2, thickness.toPx()) else Size(thickness.toPx(), size.height - padding.toPx() * 2),
					alpha = alpha,
				...
				drawRoundRect(
					...
					size =
					if (horizontal) Size(knobSize, thickness.toPx()) else Size(thickness.toPx(), knobSize),
					alpha = alpha,
