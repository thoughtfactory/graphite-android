...
fun LocalBackupScreen() {
	...
	Column(
		...
		SettingButton(text = "Take Snapshot", icon = R.drawable.ic_easy, subText = "Save everything from database") {
			scope.launch(Dispatchers.IO) {
				val tmpExportDir = File(context.cacheDir, "exportSnapshot").also {
					it.deleteRecursively()
					it.mkdirs()
				}
				val fileName = "graphite_snapshot_${System.currentTimeMillis().timeStampToPrettyNumber()}"
				Log.i("npr71", "Exporting to ${tmpExportDir.path}")

				val observer : FileObserver = object : FileObserver(File(context.cacheDir.path, "snapshot"), CLOSE_WRITE) {
...
//							copyInDirectory(File(context.attachmentDirPath()), attachmentFolder)
							...
							compress7z(tmpExportDir, sevenZOutput) { progress, total ->
								Log.d("npr71", "Compressing ${progress}/${total}")
							}

							val documentTree = DocumentFile.fromTreeUri(context, context.contentResolver.persistedUriPermissions.firstOrNull()?.uri !!)
							documentTree?.let {
								val documentFile = it.createFile("application/octet-stream", "${fileName}.7z")
								documentFile?.let {
...
//												outputFile.delete()
				...
				viewModel.getRealmSnapshot("${fileName}.realm", tmpExportDir.path)
			}
		}
				...
				snapshotList.forEach { snapshotFile ->
					SnapshotButton(fileName = snapshotFile.name ?: "") {
						scope.launch(Dispatchers.Default) {
							val tmpImportDir = File(context.cacheDir, "importSnapshot").also { file ->
								file.deleteRecursively()
								file.mkdirs()
							}
							val importFile = File(tmpImportDir, snapshotFile.name)

							context.contentResolver.openInputStream(snapshotFile.uri)?.use { inputStream ->
								importFile.outputStream().use { outputStream ->
							...
							val sevenZFile = SevenZFile(importFile).also {
								it.entries.forEach {
									Log.d("npr71", "Extracting ${it.name}")
								}
							}
							sevenZFile.entries.firstOrNull { it.name.endsWith(".realm") }?.let { realmEntry ->
								val realmFile = File(tmpImportDir, realmEntry.name)
								realmFile.outputStream().use { outputStream ->
									sevenZFile.getInputStream(realmEntry).copyTo(outputStream)
									outputStream.close()
								}
								sevenZFile.close()
								viewModel.restoreRealmSnapshot(context, realmFile.name, realmFile.parentFile.path)
							}
						}
					}
				}
