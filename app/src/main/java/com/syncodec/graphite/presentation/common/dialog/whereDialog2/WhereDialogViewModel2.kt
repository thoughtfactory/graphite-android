package com.syncodec.graphite.presentation.common.dialog.whereDialog2
...
@KoinViewModel
class WhereDialogViewModel2(repositoryStateFlow: MutableStateFlow<Repository.Companion.RepositoryStatus>) : ViewModel() {
	...
	val currentChapterId: StateFlow<RealmUUID?> = _currentChapterId
	...
	init {
		...
		viewModelScope.launch(Dispatchers.Default) {
			combine(_allChapterList, _currentChapterId) { allChapterList1, parentChapterId1 -> Pair(allChapterList1, parentChapterId1) }.collect { (allChapterList1, parentChapterId1) ->
				...
				_currentChapterPath.tryEmit(repository.value?.getChapterPath(id = parentChapterId1, includeEdge = true) ?: listOf())
			}
		}
	}
	...
	fun putNotebook(
		...
		val chapterObject = ChapterObject().apply {
			...
			this.parentId = this@WhereDialogViewModel2.currentChapterId.value
		}
		viewModelScope.launch(Dispatchers.Default) {
			repository.value?.getChapterWithParentId(parentChapterId = currentChapterId.value)?.let {
