...
fun BaseComposable(
    ...
    val wrappedRepo = koinInject<WrappedRepo>()
    val repo by wrappedRepo.repoFlow.collectAsState()
                    ...
                    componentColumnCount?.let { componentColumnCount1 ->
                        CompositionLocalProvider(
                            *providerValues,
                            ...
                            repo.let {
                                Log.d("npr71 BaseComposable", "repo : ${repo.toString()}")
                                if (it is Repo.DecryptedRepo) SecureComposable(
                                    repo = it,
                                    content = content
                                )
                            }
