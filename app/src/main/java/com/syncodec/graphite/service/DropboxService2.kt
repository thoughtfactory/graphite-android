...
//
//	override fun initSync(onStartSync : () -> Unit) {
//		super.initSync {
//			startSync(dbxClientV2)
//		}
//	}
//
...
//					checkLock(client = dbxClientV2) { driveState ->
//						when (driveState) {
//							is SyncerService.Companion.DriveState.Available -> {
//								Log.i("npr71", "DropboxService: continueSync: Available")
...
//									when (unlockResult) {
//										is SyncerService.Companion.UnlockResult.Success -> syncStatus.tryEmit(
//											SyncerService.Companion.SyncStatus.Idle(
//												syncedTimestamp = System.currentTimeMillis()
//											)
//										)
//
//										is SyncerService.Companion.UnlockResult.Failed -> null
//										is SyncerService.Companion.UnlockResult.LockExpired -> null
//									}
//								}
//								delay(30000)
//								startSync(dbxClientV2)
...
//								Log.i("npr71", "DropboxService: continueSync: Locked")
//								delay(10000)
//								startSync(dbxClientV2)
//							}
//
...
//
////					    Sync
//								sync(client = dbxClientV2, rectify = true)
//
//								delay(10000)
//								unlock(dbxClientV2, sessionId) { unlockResult ->
//									when (unlockResult) {
...
//							}
//
//							is SyncerService.Companion.DriveState.Unknown -> null
//						}
//					}
//				} ?: getDbxClient {
//					localDbxClientV2 = it
...
//				}
//			} else {
//				syncStatus.tryEmit(SyncerService.Companion.SyncStatus.Paused)
//			}
//		}
//	}
//
//	override suspend fun <T> checkLock(client : T, callback : suspend (SyncerService.Companion.DriveState) -> Unit) {
//		with(client as DbxClientV2) {
//			try {
//				files().download(Path.Lock.path).inputStream.use {
//					val jsonObject = JSONObject(it.bufferedReader().readText())
...
//				else callback(SyncerService.Companion.DriveState.Unknown)
//			} catch (e : Exception) {
//				callback(SyncerService.Companion.DriveState.Unknown)
//			}
//		}
//	}
//
//	override fun <T> lock(client : T, sessionId : String, lock : (String) -> Unit) {
//		Log.i("npr71", "DropboxService: lock()")
//		super.lock(client, sessionId) {
//			with(client as DbxClientV2) {
//				try {
//					files()
//						.uploadBuilder(Path.Lock.path)
//						.withMode(WriteMode.OVERWRITE)
//						.uploadAndFinish(it.byteInputStream())
...
//	}
//
//	override suspend fun <T> unlock(client : T, sessionId : String, callback : suspend (SyncerService.Companion.UnlockResult) -> Unit) {
//		Log.i("npr71", "DropboxService: unlock()")
//		with(client as DbxClientV2) {
//			files()
...
//			List<NoteObject>,
//			List<ChapterObject>,
//			List<BucketItemObject>,
//			List<BucketObject>,
//			List<TagObject>,
//			Set<DeletedObject>,
//		) -> Unit
//	) {
//		super.sync(client, rectify) { baseObject,
//		                              noteObjectList,
//		                              chapterObjectList,
//		                              bucketItemObjectList,
//		                              bucketObjectList,
//		                              tagObjectList,
//		                              localDeletedObjectIdList ->
//			with(client as DbxClientV2) {
...
//	}
//
//	private inline fun <reified T> DbxClientV2.downSync(id : RealmUUID, operation : SyncerService.Companion.Operation) {
...
//												bucketItemObject.getBookData()?.coverI?.let {
//													OpenLibraryApi.retrieveBookCover(it) { bitmap -> bucketItemObject.thumbnail = bitmap?.encodeBase64() }
//												}
//											}
//
//											BucketType.SHOW.name -> {
//												bucketItemObject.getShowData()?.let {
//													bucketItemObject.getShowData()?.getPosterPath()?.let { posterPath ->
//														TMDbApi.retrieveShowPoster(posterPath) { bitmap -> bucketItemObject.thumbnail = bitmap?.encodeBase64() }
//													}
//												}
//											}
//										}
...
//	}
//
//	private inline fun <reified T> DbxClientV2.upSync(id : RealmUUID, localObject : T?, operation : SyncerService.Companion.Operation) {
...
//
//		when (operation) {
//			is SyncerService.Companion.Operation.Create -> cloudSnapshot?.byteInputStream()
//				?.let {
//					upload("${Path.Root.path}/${T::class.simpleName}/${id}.json", it) {
//
//					}
//				}
//
//			is SyncerService.Companion.Operation.Update -> cloudSnapshot?.byteInputStream()
//				?.let {
//					upload("${Path.Root.path}/${T::class.simpleName}/${id}.json", it) {
//
//					}
...
//	}
//
//	private fun DbxClientV2.downloadMetadata(
//		path : String,
//		rectify : Boolean,
//		callback : (Map<RealmUUID, SyncerService.Companion.ObjectMetadata>) -> Unit
//	) {
//		if (rectify) {
...
//					Log.i("npr71", "downloadMetadata: ${fileMetadata.name}")
//					SyncerService.Companion.ObjectMetadata(
...
//	}
//
//	private fun DbxClientV2.upload(path : String, inputStream : InputStream, callback : (SyncerService.Companion.UploadResult) -> Unit) {
//		try {
//			files()
//				.uploadBuilder(path)
//				.withMode(WriteMode.OVERWRITE)
//				.uploadAndFinish(inputStream)
//			callback(SyncerService.Companion.UploadResult.Success)
//		} catch (e : NetworkIOException) {
//			callback(SyncerService.Companion.UploadResult.NetworkError)
//		} catch (e : Exception) {
//			callback(SyncerService.Companion.UploadResult.UnknownError)
//		}
//	}
//
//	private fun DbxClientV2.download(path : String, callback : (SyncerService.Companion.DownloadResult) -> Unit) {
//		return try {
//			files()
//				.download(path)
//				.inputStream
